...
namespace Umc.Core.Tools.VSGesture
{
	...
	public class VSExplorerInstaller : Installer
	{
		...
		private static void setupVS()
		{
			try
			{
				string path = System.Environment.GetEnvironmentVariable("VS80COMNTOOLS") + @"..\IDE\devenv.exe";
				Process.Start(path, "/setup").WaitForExit();
			}
			catch
			{
			}
			...
			try
			{
				string path = System.Environment.GetEnvironmentVariable("VS90COMNTOOLS") + @"..\IDE\devenv.exe";
